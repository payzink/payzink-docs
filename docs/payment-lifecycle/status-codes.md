...
Payzink API returns a standardized set of status codes to indicate the result of a transaction or payment-related
operation.

These codes help you determine whether a request was successful, requires additional customer action (e.g. 3D Secure),
or failed due to an error such as invalid card details, insufficient funds, or suspected fraud.
...
By checking the `statusCode` in the API response, your system can determine the appropriate next step (e.g. display a
message to the user, retry the transaction, prompt 3DS authentication, or block the request).
...
| `S03`       | **Success - KYC** — Transaction succeded but waiting KYC verify. (Do not deliver the service/product until you receive KYC verification.) |
...
function getErrorMessage(statusCode) {
    const messages = {
        ...
        E000014: "This payment has been declined. Please contact your bank.",
        E000015: "Card brand not supported. Please try different card.",
        E000016: "You have made too many attempts. For your security, this transaction has been temporarily blocked. Please try again after 24 hours.",
    };
...
Never automatically retry these error codes: `E000004`, `E000006`, `E000007`, `E000009`, `E000010`, `E000011`,
`E000012`. These indicate permanent issues with the card or configuration.
:::
...
Never retry transactions flagged with fraud codes (`F000000`, `F000001`, `F000002`). These should be logged, and the
transaction should be terminated immediately. Repeatedly retrying fraud-flagged transactions may result in account
suspension.
:::
